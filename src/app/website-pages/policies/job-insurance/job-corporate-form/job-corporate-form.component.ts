...
@Component({
  ...
  templateUrl: './job-corporate-form.component.html',
...
export class JobCorporateFormComponent {
  ...
  steps = [
    ...
    {
      en_title: 'Sum Insured Amount',
      ar_title: 'مبلغ التأمين الإجمالي',
      formFields: ['company_employee_total_money'],
    },
  ...
  private createFormData(): FormData {
    ...
    const fields = [
      ...
      {
        key: 'company_employee_total_money',
        value: this.claimForm.get('company_employee_total_money')?.value,
        include: this.step >= 0,
      },
      {
        key: 'need_call',
  ...
  nextStep() {
    ...
    // Special handling for step 4 (Position/Profession) - validate customJobTitle if "Other" is selected
    if (this.step === 4 && this.claimForm.get('jop_title')?.value === 'Other') {
      ...
      this.jopPolicyService
        ...
        .pipe(
          tap((response: any) => {
            ...
            this.step++;
            this.progress = (this.step + 1) * 14.3;
            if (this.step === 7) {
  ...
  private proceedWithPayment() {
    ...
    const policyData: JopPolicyDataCorporate = {
